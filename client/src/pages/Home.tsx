      ...
      <header className="sticky top-0 z-40 h-16 border-b border-[#284045]/80 bg-[#0b1316]/90 backdrop-blur-xl">
        <div className="flex h-full items-center justify-between px-4 lg:px-6">
          ...
          <div className="hidden items-center gap-7 md:flex">
            <div className="flex items-center gap-2 text-xs text-slate-400"><Activity className="h-3.5 w-3.5 text-teal-300" /><span className="font-mono-ui text-teal-100">LEARNING SYSTEM ONLINE</span></div>
        ...
        <aside className={`fixed inset-y-0 left-0 z-50 w-[284px] border-r border-[#294247] bg-[#0c1518] pt-4 transition-transform duration-200 lg:sticky lg:top-16 lg:h-[calc(100vh-64px)] lg:translate-x-0 ${sidebarOpen ? "translate-x-0" : "-translate-x-full"}`}>
          ...
          <div className="flex h-full flex-col overflow-y-auto px-4 pb-5 pt-3 lg:pt-1">
            <div className="mb-5 rounded-lg border border-[#294247] bg-[#111d20] p-3.5">
              <div className="mb-2 flex items-center justify-between"><span className="font-mono-ui text-[10px] tracking-[0.16em] text-slate-500">COURSE STATUS</span><span className="font-mono-ui text-xs text-teal-200">{overallPercent}%</span></div>
              ...
              <p className="mt-2 text-xs leading-5 text-slate-400">기초 관찰부터 종합 분석까지, 5개 레벨의 안전한 웹 보안 실습 경로입니다.</p>
            </div>
        ...
        <main className="min-w-0 flex-1 px-4 py-5 lg:px-6 lg:py-7">
          <section className="relative overflow-hidden rounded-xl border border-[#34535a] bg-[#111d20]">
            ...
            <div className="relative grid min-h-[205px] gap-6 px-5 py-6 sm:px-7 lg:grid-cols-[1fr_auto] lg:items-end lg:px-9 lg:py-8">
              <div className="max-w-2xl">
                ...
                <h1 className="text-2xl font-semibold tracking-[-0.04em] text-white sm:text-3xl">웹을 읽는 감각부터,<br className="hidden sm:block" /> 분석의 근거까지.</h1>
                <p className="mt-3 max-w-xl text-sm leading-6 text-slate-300">Hack Guidance는 초보자가 웹 요청, 응답, 쿠키, 입력값을 직접 관찰하며 보안의 기본 흐름을 익히는 분석형 학습 환경입니다.</p>
              </div>
              <div className="flex gap-3"><div className="rounded-lg border border-white/10 bg-[#0a1316]/70 px-4 py-3 backdrop-blur-sm"><p className="font-mono-ui text-[10px] tracking-wider text-slate-500">CURRENT LEVEL</p><p className="mt-1 text-sm font-semibold text-teal-100">L{activeLevel} · {levels[activeLevel - 1].label}</p></div><div className="rounded-lg border border-white/10 bg-[#0a1316]/70 px-4 py-3 backdrop-blur-sm"><p className="font-mono-ui text-[10px] tracking-wider text-slate-500">NEXT ACTION</p><p className="mt-1 text-sm font-semibold text-slate-100">문제 {String(selected.id).padStart(2, "0")} 분석</p></div></div>
            </div>
          </section>
          ...
          <section className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_350px]">
            <div className="min-w-0">
              <div className="mb-3 flex flex-wrap items-end justify-between gap-3"><div><p className="font-mono-ui text-[10px] tracking-[0.2em] text-teal-300">LEARNING QUEUE</p><h2 className="mt-1 text-lg font-semibold text-white">Level {activeLevel}. {levels[activeLevel - 1].label}</h2><p className="mt-1 text-sm text-slate-400">{levels[activeLevel - 1].description}</p></div><div className="relative"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="현재 레벨 검색" className="h-9 w-48 rounded-md border border-[#31494d] bg-[#111d20] pl-9 pr-3 text-xs text-slate-200 outline-none placeholder:text-slate-600 focus:border-teal-300/70" /></div></div>
              <div className="overflow-hidden rounded-xl border border-[#2d454a] bg-[#101b1e]">
                <div className="hidden grid-cols-[52px_minmax(0,1fr)_120px_94px] border-b border-[#294247] bg-[#142225] px-4 py-2.5 font-mono-ui text-[10px] tracking-[0.14em] text-slate-500 sm:grid"><span>ID</span><span>MODULE</span><span>TYPE</span><span>EST.</span></div>
            ...
            <aside className="panel-surface rounded-xl border border-[#345157] p-4 shadow-2xl shadow-black/15 xl:sticky xl:top-23 xl:h-fit">
              <div className="flex items-start justify-between gap-3 border-b border-[#294247] pb-4"><div><p className="font-mono-ui text-[10px] tracking-[0.18em] text-teal-300">ANALYSIS WORKSPACE</p><h2 className="mt-1 text-base font-semibold text-white">#{String(selected.id).padStart(2, "0")} {selected.shortTitle}</h2></div><span className={`rounded border px-2 py-1 font-mono-ui text-[9px] ${badgeTone[selected.difficulty]}`}>{selected.difficulty}</span></div>
