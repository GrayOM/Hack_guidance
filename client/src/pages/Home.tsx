        ...
        <main className="min-w-0 flex-1 px-4 py-5 lg:px-6 lg:py-7">
          <section className="hnet-panel hnet-hero relative overflow-hidden rounded-lg border border-[#34535a]">
            ...
            <div className="relative grid min-h-[205px] gap-6 px-5 py-6 sm:px-7 lg:grid-cols-[1fr_auto] lg:items-end lg:px-9 lg:py-8">
              <div className="max-w-2xl">
                ...
                <h1 className="text-2xl font-semibold tracking-[-0.04em] text-white sm:text-3xl">웹을 읽는 감각부터,<br className="hidden sm:block" /> 분석의 근거를 세우는 법까지.</h1>
          ...
          <section className="mt-7 grid gap-5 xl:grid-cols-[minmax(0,1fr)_350px]">
            <div className="min-w-0">
              <div className="mb-3 flex flex-wrap items-end justify-between gap-3"><div><p className="font-mono-ui text-[10px] tracking-[0.2em] text-teal-300">LEARNING QUEUE</p><h2 className="mt-1 text-lg font-semibold text-white">Level {activeLevel}. {levels[activeLevel - 1].label}</h2><p className="mt-1 text-sm text-slate-400">{levels[activeLevel - 1].description}</p></div><div className="relative"><Search className="absolute left-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-slate-500" /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="이 레벨의 문제 검색" className="h-9 w-48 rounded-md border border-[#31494d] bg-[#111d20] pl-9 pr-3 text-xs text-slate-200 outline-none placeholder:text-slate-600 focus:border-teal-300/70" /></div></div>
