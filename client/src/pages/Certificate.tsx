...
export default function Certificate() {
  ...
  return <div className="hacknet-shell min-h-screen bg-[#060b0d] text-[#e7f2ef]"><header className="hnet-header border-b border-[#294247]"><div className="mx-auto flex h-16 max-w-5xl items-center justify-between px-4 sm:px-6"><button onClick={() => setLocation("/")} className="inline-flex items-center gap-2 text-sm text-slate-300 transition hover:text-teal-100"><ArrowLeft className="h-4 w-4" />학습 대시보드</button><button onClick={() => setLocation("/verify")} className="font-mono-ui text-[10px] tracking-[0.12em] text-teal-200 transition hover:text-teal-100">PUBLIC_VERIFY</button></div></header><main className="mx-auto max-w-5xl px-4 py-10 sm:px-6"><p className="font-mono-ui text-[10px] tracking-[0.2em] text-teal-300">HACK // GUIDANCE · WEB SECURITY FUNDAMENTALS</p><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em] text-white">수료 기록 상태 확인</h1><p className="mt-3 max-w-2xl text-sm leading-6 text-slate-400">실습, 레벨 평가, 방어 기준 학습이 모두 기록되면 인증번호가 포함된 수료 기록을 발급할 수 있습니다. 이는 학습 완료 기록이며 공인 자격증이 아닙니다.</p><div className="mt-8 grid gap-4 sm:grid-cols-3"><Requirement icon={<FileCheck2 className="h-4 w-4" />} label="학습 문제" value={`${completed} / 50`} met={completed >= 50} /><Requirement icon={<ShieldIcon />} label="방어 기준" value={`${defense} / 50`} met={defense >= 50} /><Requirement icon={<Award className="h-4 w-4" />} label="레벨 평가" value={`${assessments} / 5`} met={assessments >= 5} /></div><section className="hnet-panel relative mt-8 overflow-hidden border border-[#335158] p-6 sm:p-8"><img src="/manus-storage/hg-certificate-mark_099b5170.jpg" alt="" className="absolute inset-0 h-full w-full object-cover opacity-[0.07] mix-blend-screen" /><div className="relative"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="font-mono-ui text-[10px] tracking-[0.18em] text-teal-300">CERTIFICATE STATUS</p><h2 className="mt-2 text-xl font-semibold text-white">{certificate ? "수료 기록이 발급되었습니다." : eligible ? "수료 요건이 확인되었습니다." : "수료 요건을 추적 중입니다."}</h2></div><span className={`rounded-none border px-2.5 py-1 font-mono-ui text-[10px] ${certificate || eligible ? "border-teal-300/35 bg-teal-300/10 text-teal-100" : "border-[#466064] text-slate-400"}`}>{certificate ? "ISSUED" : eligible ? "READY" : "IN_PROGRESS"}</span></div>{certificate ? <div className="hnet-terminal mt-7 border border-teal-300/25 p-5"><div className="flex items-center gap-2 text-teal-100"><KeyRound className="h-4 w-4" /><span className="font-mono-ui text-xs tracking-[0.12em]">VERIFICATION CODE</span></div><p className="mt-3 font-mono-ui text-2xl tracking-[0.08em] text-white">{certificate.certificateCode}</p><p className="mt-3 text-sm text-slate-400">발급일: {new Date(certificate.issuedAt).toLocaleDateString("ko-KR")}</p><button onClick={() => setLocation(`/certificate/print/${certificate.certificateCode}`)} className="mt-5 inline-flex items-center gap-2 border border-teal-300/40 px-3 py-2 text-xs font-medium text-teal-100 transition hover:bg-teal-300/10"><FileCheck2 className="h-3.5 w-3.5" />수료증 보기 및 인쇄</button></div> : <button onClick={() => issue.mutate()} disabled={issue.isPending} className="mt-7 inline-flex items-center gap-2 bg-teal-300 px-4 py-2.5 text-sm font-semibold text-[#092024] transition hover:bg-teal-200 disabled:cursor-not-allowed disabled:opacity-60"><CheckCircle2 className="h-4 w-4" />{issue.isPending ? "수료 요건 확인 중" : "수료 기록 발급 가능 여부 확인"}</button>}</div></section></main></div>;
}
